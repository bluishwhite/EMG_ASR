...
from src.data.dataset import EMGDataset, TextLineDataset, ZipDataset,EMGNPYVocieDataset
from src.decoding import beam_search,beam_search_ctc, ctc_decode_beam_search,beam_for_ctc
import torch
import numpy as np
from src.modules.criterions import CtcCriterion
...
def kl_anneal_function(step, k=0.0025, x0=5000):
...
class EMGVoiceNPYMIXCTCHelper(object):
    def __init__(self, model, tgt_vocab, configs, use_cuda):
        ...
        self.mix_alpha = training_configs['mixup']
        
        
        self.critic = CtcCriterion(blank_id=tgt_vocab.blank_id())
        ...
        train_bitext_dataset = ZipDataset(
            EMGNPYVocieDataset(data_path=data_configs['train_data'][0],nfft=data_configs['nfft'],
                        hop_length=data_configs['hop_length'],norm=data_configs['norm'],ss=data_configs['ssfilter'],
                        voice=model_configs['voice']
                            ),
    ...
    def mixup_data(self,weis, x, y,alpha=1.0, use_cuda=True):
        ...
        weis = weis.unsqueeze(1)
        mixed_wights = lam * weis + (1-lam) * weis[index, :]
        ...
        return mixed_wights.squeeze(1), mixed_x,ctc_ya, ctc_yb, ya_lengths, yb_lengths, lam
    ...
    def pair_data_variable(self, features, seqs_y_t):
        ...
        weighs = []
        features_length = [feature.shape[0] for weight, feature in features]
        ctc_targets = []
        ...
        weighs = torch.FloatTensor(weighs)

        
        if self.use_cuda:
            padded_features = padded_features.cuda(self.device)
            tgt_words = tgt_words.cuda(self.device)
            weighs = weighs.cuda(self.device)



        return weighs, padded_features, tgt_words
    ...
    def compute_forward(self,weighs, seqs_x, seqs_y, norm,step):
        y_inp = seqs_y[:, :-1].contiguous()
        y_label = seqs_y[:, 1:].contiguous()



        mix_weighs, inputs, ctc_ya, ctc_yb, ctc_ya_length, ctc_yb_length, lam = self.mixup_data(weighs, seqs_x, seqs_y,self.mix_alpha, use_cuda=True)
        ...
        with torch.enable_grad():
            ...
            _,ctc_mask, encoder_log_pro_a, encoder_output_length_a = self.model(inputs)
            _,ctc_mask, encoder_log_pro_b, encoder_output_length_b = self.model(inputs)

            loss_a = self.critic(inputs=encoder_log_pro_a.transpose(1,0),labels=ctc_ya,input_lengths=encoder_output_length_a,target_lengths=ctc_ya_length,reduce=False,normalization=norm)
            loss_b = self.critic(inputs=encoder_log_pro_b.transpose(1,0),labels=ctc_yb,input_lengths=encoder_output_length_b,target_lengths=ctc_yb_length,reduce=False,normalization=norm)
            
            loss = lam *  loss_a + (1 - lam) * loss_b   

            loss = torch.mul(mix_weighs,loss)

            loss = loss.sum()
        torch.autograd.backward(loss)

        mask = y_label.detach().ne(self.tgt_pad)
        # pred = encoder_log_pro.detach().max(2)[1]  # [batch_size, seq_len]
    ...
    def train_batch(self, seqs_x_t, seqs_y_t, norm,step):
        ...
        weighs, src_words, tgt_words = self.pair_data_variable(feature, seqs_y)
        
        stat = self.compute_forward(weighs, src_words, tgt_words, norm,step)
    ...
    def ctc_beamsearch_batch(self, seqs_x_t,beamdecoder, beam_size, max_steps, len_pen):
        ...
        with torch.no_grad():
            ...
            #                        src_seqs=pad_features,
            #                        alpha=len_pen)
            word_ids =beam_for_ctc(nmt_model=self.model, 
                                    decoder=beamdecoder,
                                   beam_size=beam_size,
                                   src_seqs=pad_features,
                                   alpha=len_pen)
