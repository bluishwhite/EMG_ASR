...
from src.emg_ctc_helper import *
...
def train(nmt, configs, args):
    ...
    for iter in range(3000):
        ...
        if iter % training_configs['epcoh_valid_freq'] == 0 and iter!= 0: 
            ...
            train_cer = evaluate_train(nmt, training_configs, data_configs)
            print("step %d, epoch %d: train cer : %.5f, time %.2f" \
                    % (global_step, iter, train_cer, during_time))
...
def evaluate_train(nmt, training_configs, data_configs):
    batch_size = training_configs['valid_batch_size']

    #valid_dataset = TextLineDataset(data_path=data_configs['valid_data'][0])

    valid_bitext_dataset = ZipDataset(
        EMGDataset(data_configs['train_data'][0],nfft=data_configs['nfft'],
                        hop_length=data_configs['hop_length'],norm=data_configs['norm'],
                        ss=data_configs['ssfilter']
                     ),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_path', type=str, default="save/1238sentences/mixup/transformer_ctc/dataauguement_alltest_mix0025/config.yml",
    ...
    nmt = EMGCTCHelper(nmt_model, tgt_vocab, configs, use_cuda)
