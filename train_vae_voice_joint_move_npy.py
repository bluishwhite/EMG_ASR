...
from src.data.dataset import MoveNodeEMGDataset
from src.data.vocabulary import Vocabulary
from src.data.vocabulary_transducer import Vocabulary_T
from src.models import build_model
from src.utils.common_utils import *
from src.vae_emg_voice_npy_helper import *
...
def train(nmt, configs, args,ctc_beam_decoder):
        ...
        for batch in training_iter:
            ...
            try:
                ...
                for seqs_x_t, seqs_y_t in split_shard(seqs_x, seqs_y, split_size=training_configs['update_cycle']):
                    ...
                    stat = nmt.train_batch(seqs_x_t, seqs_y_t, n_samples_t, iter, iter)
            ...
            if global_step % training_configs['loss_valid_freq'] == 0:
                ...
                dev_cer = evaluate(nmt, training_configs, data_configs,ctc_beam_decoder)
...
def evaluate(nmt, training_configs, data_configs,ctc_beam_decoder):
    ...
    for xs in valid_iter:
        ...
        sub_trans = nmt.ctc_beamsearch_batch(seqs_x,ctc_beam_decoder, beam_size, max_steps, alpha)
    ...
    total_cer = float(total_cer/len(truth))
...
def test(nmt, configs, args,ctc_beam_decoder):
    ...
    valid_bitext_dataset = ZipDataset(
        MoveNodeEMGDataset(data_configs['test_data'][0],nfft=data_configs['nfft'],
                        hop_length=data_configs['hop_length'],norm=data_configs['norm'],
                        ss=data_configs['ssfilter'],
                        point_detection=point_detection_state,move_channel=data_configs['move_channel']
                     ),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default="Conv2dTransformer",
    ...
    nmt = VaeEMGASRDecayHelper(nmt_model, tgt_vocab, configs, use_cuda)
