...
def beam_search_ctc(nmt_model, beam_size, src_seqs, alpha=-1.0):
    nmt_model.eval()
    enc_outputs = nmt_model.encode(src_seqs)
    probs = enc_outputs['encoder_log_pro'].transpose(0, 1)
    
    _, decoded = torch.max(probs, 2)
    decoded = decoded.view(decoded.size(0), decoded.size(1))
        # convert digit idx to chars
    # ctc_decoder = ctcBeamSearch (beam_size, blank_index = nmt_model.tgt_vocab.blank_id())

    # 
    # frame_seq_len = enc_outputs['encoder_output_length'].data.cpu().tolist()
    # res = ctc_decoder.decode(probs, frame_seq_len)

    return decoded.transpose(0,1).unsqueeze(1)
