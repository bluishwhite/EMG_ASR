...
from src.data.dataset import TextLineDataset, ZipDataset,EMGDataset,EMGNPYVocieDataset
...
class EMGAddVoiceNPYCTCHelper(object):
    ...
    def compute_forward(self, weights, seqs_x, seqs_y,ctc_y, ctc_lengths, norm, step):
        ...
        with torch.enable_grad():
            ...
            _,ctc_mask, encoder_log_pro, encoder_output_length = self.model(seqs_x)

            loss = self.critic(inputs=encoder_log_pro.transpose(1,0),labels=ctc_y,input_lengths=encoder_output_length,target_lengths=ctc_lengths,reduce=False,normalization=norm)
    ...
    def train_batch(self, seqs_x_t, seqs_y_t, norm, step):
        ...
        stat = self.compute_forward(weights, src_words, tgt_words,ctc_targets,tgt_lengths, norm, step)
