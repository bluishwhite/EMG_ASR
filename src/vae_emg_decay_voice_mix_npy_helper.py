...
class VaeEMGVoiceMIXNPYDecayHelper(object):
    def __init__(self, model, tgt_vocab, configs, use_cuda):
        ...
        train_bitext_dataset = ZipDataset(
            EMGNPYVocieDataset(data_path=data_configs['train_data'][0],nfft=data_configs['nfft'],
                        ...
                        ss=data_configs['ssfilter'],voice=model_configs['voice'],
    ...
    def mixup_data(self,weights, x, y,seqs_tgts,alpha=1.0, use_cuda=True):
        ...
        mix_w = lam*weights + (1-lam)* weights[index, :]
        ...
        return mix_w, mixed_x, y_a, y_b, ctc_ya, ctc_yb, ya_lengths, yb_lengths, lam
    ...
    def pair_data_variable(self, features, seqs_y_t):
        ...
        wights =  torch.FloatTensor(wights)
        ...
        if self.use_cuda:
            ...
            wights = wights.cuda(self.device)


        return wights, padded_features, tgt_words,seqs_y_t
    ...
    def compute_forward(self, weights,seqs_x, seqs_y,seqs_tgt, norm,iter):
        mix_w,inputs, targets_a, targets_b, ctc_ya, ctc_yb, ctc_ya_length, ctc_yb_length, lam = self.mixup_data(weights.unsqueeze(1), seqs_x, seqs_y,seqs_tgt,self.mix_alpha, use_cuda=True)
        ...
        with torch.enable_grad():
            ...
            loss = lam * loss_a + (1 - lam) * loss_b +  self.att_weight*loss_kl_anneal_function(iter)*(lam * a_KLD +  (1 - lam) *b_KLD )
            loss = torch.mul(mix_w.squeeze(1), loss)
            loss = loss.sum()
    ...
    def train_batch(self, seqs_x_t, seqs_y_t, norm,iter):
        ...
        wights, src_words, tgt_words,seqs_tgts = self.pair_data_variable(feature, seqs_y)
        
        stat = self.compute_forward(wights, src_words, tgt_words,seqs_tgts, norm,iter)
