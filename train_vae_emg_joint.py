...
from src.vae_emg_decay_helper import *
import re
...
def train(nmt, configs, args,ctc_beam_decoder):
    ...
    # contine_train = Saver(save_prefix="{0}.ckpt".format(os.path.join('save/1238sentences/transformer_joint/onlysilent_0.2vae_epoch_weight0.8/', args.model_name)),
    #                          num_max_keeping=training_configs['num_kept_checkpoints']
    #                          )
    # contine_train.load_latest(model=nmt.model, optim=optim, lr_scheduler=scheduler)
    best_cer = 100
...
def test(nmt, configs, args,vocab_list,ctc_beam_decoder):
    ...
    valid_bitext_dataset = ZipDataset(
        EMGDataset(data_configs['test_data'][0],nfft=data_configs['nfft'],
                        hop_length=data_configs['hop_length'],norm=data_configs['norm'],
                        ss=data_configs['ssfilter'],point_detection=point_detection_state
                     ),
...
if __name__ == '__main__':
    ...
    nmt = VaeEMGASRHelper(nmt_model, tgt_vocab, configs, use_cuda)

    if args.evaluate:

        print('start evaluate...')
        cer = test(nmt, configs, args)
        print("the cer", cer)
        out_info = ("cer:  %.5f") % (cer)
        print(out_info)
    else:
