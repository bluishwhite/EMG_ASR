...
from src.data.dataset import TextLineDataset, ZipDataset, EMGAddVoiceDataset,EMGDataset
from src.decoding import beam_search,beam_search_ctc,beam_for_ctc
import torch
import numpy as np
from src.modules.criterions import JointCtcAttention,NMTCriterion
...
class VaeEMGASRDecayHelper(object):
    def __init__(self, model, tgt_vocab, configs, use_cuda):
        ...
        self.voice_weight = model_configs['voice']
        if self.joint_ctc:
            self.critic = JointCtcAttention(blank_id=tgt_vocab.blank_id(),padding_idx=self.tgt_pad)
        else:
        ...
        train_bitext_dataset = ZipDataset(
            EMGAddVoiceDataset(data_path=data_configs['train_data'][0],nfft=data_configs['nfft'],
                        ...
                        ss=data_configs['ssfilter'],voice=self.voice_weight,
    ...
    def pair_data_variable(self, features, seqs_y_t):
        ...
        features_length = [feature.shape[0] for weig, feature in features]
        ...
        return weighs,padded_features, tgt_words, ctc_tgt_lengths,ctc_targets
    ...
    def compute_forward(self, weights, seqs_x, seqs_y,ctc_y, ctc_lengths, norm, iter, step):
        ...
        with torch.enable_grad():
            ...
            log_probs, encoder_log_pro, encoder_output_length, mu, log_var = self.model(seqs_x, y_inp)
            ...
            KLD = torch.sum(KLD_element).mul_(-0.5)
            if self.joint_ctc:
                closs = self.critic(inputs=log_probs, labels=y_label, encoder_output=encoder_log_pro.transpose(1,0), ctc_targets=ctc_y, input_lengths=encoder_output_length, target_lengths=ctc_lengths, weight=att_weight, reduce=False, normalization=norm)
            else:
                closs = self.critic(inputs=log_probs, labels=y_label, reduce=False, normalization=norm)
            loss = closs + self.att_weight*loss_kl_anneal_function(step) * KLD
    ...
    def train_batch(self, seqs_x_t, seqs_y_t, norm,iter,step):
        self.model.train()

        feature = [x_t for x_t in seqs_x_t ]
        seqs_y = [self.tgt_data_id(y_t) for y_t in seqs_y_t]
        weights, src_words, tgt_words,tgt_lengths,ctc_targets = self.pair_data_variable(feature, seqs_y)
        
        stat = self.compute_forward(weights, src_words, tgt_words,ctc_targets,tgt_lengths, norm,iter,step)
